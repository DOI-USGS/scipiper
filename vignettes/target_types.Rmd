...
`scipiper` targets can be indicator files with a [shared cache](shared_cache.html), local data files with or without local indicator files, or simply R objects.  This vignette provides some guidelines for when to use these different patterns.
...
**Local data file only**:  The target will be rebuilt if the local data file does not exist or depedencies have changed.  For cases where a target takes a relatively long time to build, but creates a massive file that is more cumbersome to upload/download than rebuild.  

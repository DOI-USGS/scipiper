...
You can now create a task_plan that wraps the tasks and steps together. In the following call to `create_task_plan`, a couple of things happen. First, the steps you identified are organized into an R object that `scipiper` can use efficiently later. Second, when the default argument `add_complete=TRUE` is accepted (e.g., by not specifying `add_complete` at all), a final step gets tacked onto the list. The final step generates an .ind file for each task once all other steps for that task have been completed (or, if `final_steps` is specified, once those specified steps have been completed).

The main benefit of this final step and .ind file is speed: iteration over the complete task plan can be more efficient because the presence of an .ind file for a task can be used as a first-cut indication that the task doesn't need to be built, and the iteration effort can be more efficiently spent on other tasks. The usual way that `remake` determines which tasks need to be rebuilt can be quite slow for large task lists, so speed here is a good thing.
```{r plan}
...
Note that at the end of each state's task list is a final task, e.g., `states/log/WI.ind`, that we didn't directly specify in the task_plan. These recipes for the `complete` step were generated in `create_task_plan` as described above.
...
To use the baked-in retries effectively, simply ensure that the functions defined in your task-steps throw errors (e.g., with `stop()`) when things go wrong. No need to build retries into the functions themselves because `loop_tasks` will take care of that for you. And if your function isn't the sort that would benefit from retries - e.g., HTTP requests need retries, but a deterministic algorithm should just give up if it breaks - you can change the value of the `num_tries` argument to `loop_tasks`.

One drawback of the `add_complete` system of .ind files (described above) is that the speed shortcut only works if the presence of .ind files is a fairly accurate description of which tasks are up to date. If you build a complete task plan once, change an input, and intend to rebuild, then it's strongly recommended that you delete the .ind files before calling `loop_tasks` again. If you don't, nothing will break, but your build will be slower and will not include automatic retries.
...
If we try to `scmake` any of the targets in the task makefile without changing any of the inputs, `remake` correctly recognizes that they're up to date already, and the target is not rebuilt. Instead, `AZ_plot.png` is already `OK`:
```{r try_rebuild_one}
...
One way to force a complete rebuild (not generally a good use of time, but sometimes necessary) is to use `scdel` with the `target_names` argument set to `NULL`. We'll do this several times in this vignette for demonstration purposes.
```{r delete_all}
scdel(target_names=NULL, remake_file='task_plan_1.yml')
```

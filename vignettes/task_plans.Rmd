...
You'll need the `task_plan` R object for two things: to create a remake .yml makefile and to run `loop_tasks`. First, the makefile:
...
Next, use the `task_plan` and the `task_makefile` in `loop_tasks` to run all the steps in all the tasks:
